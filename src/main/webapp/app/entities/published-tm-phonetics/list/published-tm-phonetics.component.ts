...
export class PublishedTmPhoneticsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPublishedTmPhonetics): number =>
    this.publishedTmPhoneticsService.getPublishedTmPhoneticsIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.publishedTmPhonetics || this.publishedTmPhonetics.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.publishedTmPhonetics = this.refineData(dataFromBody);
  }
