...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'createdDate' | 'modifiedDate' | 'joiningDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      createdDate: employee.createdDate?.toJSON() ?? null,
      modifiedDate: employee.modifiedDate?.toJSON() ?? null,
      joiningDate: employee.joiningDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      createdDate: restEmployee.createdDate ? dayjs(restEmployee.createdDate) : undefined,
      modifiedDate: restEmployee.modifiedDate ? dayjs(restEmployee.modifiedDate) : undefined,
