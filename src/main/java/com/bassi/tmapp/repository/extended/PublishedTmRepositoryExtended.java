...
public interface PublishedTmRepositoryExtended extends JpaRepository<PublishedTm, Long>, JpaSpecificationExecutor<PublishedTm> {
    @Query(
        ...
        "SELECT tm.name as published, t.name as registeredTrademark FROM published tm  INNER JOIN registered t on tm.phonetic_pk = t.phonetic_pk",
        nativeQuery = true
    )
    ...
    @Query(
        value = "select tm.* FROM published_tm tm where name is not  null and journal_no = ?1 " +
        "and tm.id NOT IN (SELECT published_tm_id FROM published_tm_phonetics)",
        nativeQuery = true
    )
    ...
    @Query(
        value = "SELECT * FROM published_tm  WHERE application_no = ?1 AND tm_class = ?2 AND journal_no = ?3 LIMIT 1",
        nativeQuery = true
    )
