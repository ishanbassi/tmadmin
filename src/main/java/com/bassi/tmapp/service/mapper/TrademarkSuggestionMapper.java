...
import com.bassi.tmapp.service.TrademarkFaqUtil;
...
public interface TrademarkSuggestionMapper extends EntityMapper<TrademarkDTO, Trademark> {
    ...
    @AfterMapping
    default void setSeoFields(@MappingTarget TrademarkDTO dto, Trademark entity) {
        String slug = SlugUtil.generate(entity.getName(), entity.getTmClass(), entity.getApplicationNo(), entity.getType());

        dto.setSlug(slug);
        dto.setUrl("/trademarks/" + slug);

        List<FaqItem> faqs = TrademarkFaqUtil.generate(entity);
        dto.setFaqs(faqs);
    }
}
